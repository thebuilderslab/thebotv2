...
VALUES (
  ...
  'submit_code_change',
  'http_api',
  'Submit a code change for deployment. Provide the modified file(s) and a commit message. The change is applied to the repo and deployed to Cloudflare Workers automatically. Results are reported back via Telegram. IMPORTANT: always read the current file first using read_github_file before submitting a change.',
  '{"type":"object","properties":{"files":{"type":"array","description":"Array of files to write","items":{"type":"object","properties":{"path":{"type":"string","description":"Repo-relative path e.g. packages/backend-api/src/routes/finance.ts"},"content":{"type":"string","description":"Full new file content (not a diff — complete file)"}},"required":["path","content"]}},"commit_message":{"type":"string","description":"Concise git commit message describing the change (imperative tense, under 72 chars)"}},"required":["files","commit_message"]}',
